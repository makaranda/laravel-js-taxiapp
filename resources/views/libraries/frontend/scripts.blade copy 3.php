            ...
            success: function(data) {
                if (data.status == 1 && data.booking == 1) {
                    ...
                    if (!window.mapInitialized) {
                        ...
                        // map_location = L.map('showMap').setView(pickupCoords, 13);
                        //var map_location = L.map('showMap');
                        var map_location = L.map('showMap').setView(pickupCoords, 12);

                        // Add OpenStreetMap tiles
                        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                            attribution: '© OpenStreetMap contributors',
                            maxZoom: 18  // Ensure the max zoom level is properly defined
                        }).addTo(map_location);


                        //alert();
                        //var bounds = L.latLngBounds([pickupCoords, dropoffCoords]);
                        //map_location.fitBounds(bounds);

                        // Use OpenStreetMap tiles or any other tile provider
                        // L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                        //     attribution: '© OpenStreetMap contributors',
                        //     maxZoom: 18,
                        //     tileSize: 512,
                        ...
                        // var pickupPopup = L.popup({ closeOnClick: false, autoClose: false })
                        //     .setLatLng(pickupCoords)
                        //     .setContent("Pickup Point")
                        //     .openOn(map_location);

                        // var dropoffPopup = L.popup({ closeOnClick: false, autoClose: false })
                        //     .setLatLng(dropoffCoords)
                        //     .setContent("Drop-Off Point")
                        //     .openOn(map_location);
                            ...
                            success: function(data) {
                                ...
                                if (data.routes && data.routes.length > 0) {
                                    ...
                                    try {
                                        ...
                                        var pickupMarker = L.marker(pickupCoords).addTo(map_location);
                                        var dropoffMarker = L.marker(dropoffCoords).addTo(map_location);
